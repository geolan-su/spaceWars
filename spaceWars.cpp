...
void reinitLasers(laser playerLasers[], laser enemyLasers[]);
    ...
    while (!stop) {
                ...
                if(GAMEOVER) {
                        ...
                        case SDL_SCANCODE_X: {
                         ...
                         //printf("initial steve x: %i", steve.rect.x);
                            ...
                            reinitLasers(playerLasers, enemyLasers);                            GAMEOVER = 0;
        ...
        */
        ...
        if(steve.hp <= 0 && !godmode) {
            ...
            SDL_Surface* textSurface = TTF_RenderText_Shaded(font, "Press x to continue or esc to quit.", fgColor, bgColor);
            SDL_Texture* textText = SDL_CreateTextureFromSurface(renderer, textSurface);
            int text_width = textSurface->w;
            int text_height = textSurface->h;
            SDL_FreeSurface(textSurface);
            //SDL_Rect renderQuad = {20, WINDOW_HEIGHT - 30, text_width, text_height};
            SDL_Rect messageRect;
            messageRect.x = 0;
            messageRect.y = 0;
            messageRect.w = 100;
            messageRect.h = 100;
            //SDL_QueryTexture(textText, NULL, NULL, &text_width, &text_height);
            SDL_RenderCopy(renderer, textText, NULL, &messageRect);
